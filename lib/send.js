...
const { insert } = require('../lib/history')
...
class Send {
  constructor (config) {
    ...
    switch (true) {
      case (filePath || '').substr(0, 4) === 'http':
        this.config.source = 'HTTP'
        this.byHttpImage()
        break

      case fromClipboard:
        this.config.source = 'Clip'
        this.byClipImage()
        break
  ...
  async upload (name, content) {
    try {
      ...
      let res
      if (this.config.method === 'Imgur') {
        res = await imgurUpload(content)
      } else if (this.config.method === 'GitLab') {
  ...
  async byClipImage () {
    try {
      ...
      const info = imageInfo(fileContent)
      const fileName = this.getFileName('', `.${info.format.toLowerCase()}`)
